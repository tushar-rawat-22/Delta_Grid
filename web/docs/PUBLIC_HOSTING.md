...
Its verifier, `web/scripts/verify-live-boundary.sh`, checks the live system from an anonymous Internet client. It requires the public homepage, sanitized Research Demo, expected security headers and public robots policy to remain available. It also fails if known founder-only markers appear in public HTML. On the private Worker it checks four independent surfaces: the Founder research workspace, Founder control plane, Founder research API, and machine-agent API. Every anonymous request must either be redirected to Cloudflare Access or fail with `401`/`403`; any other response is treated as an isolation regression. The machine-path probe uses `GET` deliberately even though the Worker accepts only authenticated `POST`, so a missing edge Service Auth boundary would surface as the Worker's `405` and fail the monitor instead of being mistaken for healthy isolation.

This monitor is intentionally a boundary detector, not an authenticated Founder Mode test. It must never obtain credentials just to make the check more comprehensive: the security property being tested is that an anonymous observer cannot receive founder content or reach a private Worker surface directly.
