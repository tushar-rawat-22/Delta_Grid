...
export async function verifyResearchCsrf(
  ...
  if (!Number.isSafeInteger(expires) || expires < Math.floor(now / 1000) || expires > Math.floor(now / 1000) + TOKEN_TTL_SECONDS + 30) return false;
...
export function verifySameOrigin(request: Request): boolean {
  ...
  return origin === url.origin && (fetchSite === "same-origin" || fetchSite === "none");
}
